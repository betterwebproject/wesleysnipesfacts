  ...
  <xsl:template match="/">
    <html lang="en-US">
      <head>         
        ...
        <style>main{max-width:56ch}.info{margin-block-start:var(--big-gap);margin-block-end:4.06rem}.subscribe{width:fit-content;padding:.75rem;margin-block-start:2rem;border:1px solid var(--border);border-radius:1rem;box-shadow:0 2px 6px rgba(0,0,0,.05);font-family:monospace;font-size:calc(.9rem + .15vw);word-break:break-all;background:var(--post-background)}.item{padding:var(--big-gap) var(--gap);border:1px solid var(--border);border-radius:1rem;box-shadow:0 2px 6px rgba(0,0,0,.05);background-color:var(--post-background)}.item+.item{margin-block-start:calc(var(--big-gap) * 2)}.item-title{font-family:var(--heading);font-size:var(--heading-1)}.tags{display:flex;flex-wrap:wrap}.tag{color:var(--accent)}</style>
      </head>
      <body>

  ...
  <xsl:template match="/">
    <html lang="en-US">
      <head>         
        ...
        <style>.intro{margin-block-end:var(--space-3)}.item>*+*{margin-block-start:var(--space-1)}.subscribe{width:max-content;padding:.75rem 1rem;margin-block-start:var(--space-2);border:1px solid var(--border);border-radius:1rem;box-shadow:0 2px 6px rgba(0,0,0,.05);font-family:monospace;font-size:calc(.9rem + .15vw);word-break:break-all;background:var(--post-background);cursor:pointer;transition:background .2s ease}@media(hover:hover) and (pointer:fine){.subscribe:hover{background:var(--border)}}.item{position:relative;padding:var(--space-2) 2.25rem;border:1px solid var(--border);border-radius:1rem;box-shadow:0 2px 6px rgba(0,0,0,.05);background-color:var(--post-background)}.item+.item{margin-block-start:var(--space-3)}.item-title{font-family:var(--heading);font-size:var(--heading-1)}.tags{display:flex;flex-wrap:wrap}.tag{color:var(--accent)}@media(max-width:640px){.item{padding:var(--space-2) 1.5rem}}</style>
      </head>
      <body>
        <a href="#main-content" class="skip-link">Skip to content</a>
        <header class="header-small">
          <div class="header-image">
            <span class="vh">Image of Wesley Snipes, with Kwanzaa facepaint and Egyptian rod, in a jail cell for tax evasion</span>
          </div>
          <a href="/">
            <svg:svg class="arrow" viewBox="0 0 25 21.4" aria-hidden="true">
            ...
            <span class="site-title-small">
              Wesley Snipes Facts<span class="sup">TM</span>
            </span>
          </a>
        </header>
